...
DRRgenerator::DRRgenerator() {
  ...
  // distance of the camera from the center of the CT coordinate system (after
  // translation)
...
#pragma omp parallel for
  for (int r = 0; r < color.rows; ++r)
    for (int c = 0; c < color.cols; ++c) {
      ...
      double texitpoint;
      double tentrypoint;
      // we search for the entry point and exit point to avoid useless
      // iterations
      ...
      if (count > 0) {
        double val =
            sum / count;  // we average over the number of passed voxels
        color_raw.at<double>(r, c) = val;
      }
    }
  // the value inside color_raw are actually very small, so we scale it in
  // between 0 and 255
  double min;
  double max;
...
Eigen::Isometry3f DRRgenerator::cv2eigeniso(cv::Mat transfo) {
  ...
  for (size_t i = 0; i < 4; i++)
    for (size_t j = 0; j < 4; j++) mat4f(i, j) = transfo.at<double>(i, j);
  mat.matrix() = mat4f;
  return mat;
}
/* when requesting the CT value at one point, we perform trilinear interpolation
 ...
 * it looks at the intersection of the ray with the 6 planes making the outter
 * surface of the CT box only 2 planes will intersect the ray in the limit of
 * the CT dimension
 */

void DRRgenerator::findentryandexitpoint(Vector3f startpoint, Vector3f ray,
                                         double &texitpoint,
                                         double &tentrypoint) {
  float sx = CTvol.size_x * CTvol.voxel_size_x / 2;
  float sy = CTvol.size_y * CTvol.voxel_size_y / 2;
  ...
  tentrypoint = tmin;  // thr lowest value means it is closer to the starting
                       // point, therefore it is the entry point
  texitpoint = tmax;
}
